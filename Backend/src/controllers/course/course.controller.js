...
const fetchAllCourses=asyncHandler(async (req,res,next)=>{
    ...
    const {perPage,skip,totalPages}=await getPaginationData("course",pageNumber || 1,limit || 10);
    console.log({perPage,skip,totalPages})
    ...
    return res.status(200).json(new ResponseConfig(200,"All courses fetched successfully",{courses,totalPages}));
});
