...
const getUsers = asyncHandler(async (req, res, next) => {
  ...
  const {perPage,skip,totalPages}=await getPaginationData("user",pageNumber || 1,limit || 10);
  let users = await prisma.user.findMany({
 skip,
 take:perPage
  });
  if (users.length <= 0) return next(new ErrorConfig(404, "users not found"));
  return res.status(200).json(new ResponseConfig(200, null, {users,totalPages}));
});

const updateProfile = asyncHandler(async (req, res, next) => {
  const { id } = req.loggedInfo;
  ...
  if (updateUser)
    return res
      .status(202)
      .json(new ResponseConfig(202, "updated successfully", updateUser));
